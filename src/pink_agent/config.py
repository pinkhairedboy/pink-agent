...
# Verbose mode (for singleton and other utilities)
...
# Development: full verbose logging (DEBUG)
log_level = logging.DEBUG if DEV_MODE else logging.INFO
logging.basicConfig(
    format='%(asctime)s - %(name)s - %(levelname)s - %(message)s' if DEV_MODE else '%(asctime)s - %(message)s',
    datefmt='%Y-%m-%d %H:%M:%S',
    level=log_level
)
